...
*/
int32_t GENERIC_CSS_CommandDevice(int32_t handle, uint8_t cmd_code, uint32_t payload)
{
    int32_t status = OS_SUCCESS;
    int32_t bytes = 0;
    uint8_t write_data[GENERIC_CSS_DEVICE_CMD_SIZE] = {0};
    uint8_t read_data[GENERIC_CSS_DEVICE_DATA_SIZE] = {0};

    payload = CFE_MAKE_BIG32(payload);

    /* Prepare command */
    write_data[0] = GENERIC_CSS_DEVICE_HDR_0;
    write_data[1] = GENERIC_CSS_DEVICE_HDR_1;
    write_data[2] = cmd_code;
    write_data[3] = payload >> 24;
    write_data[4] = payload >> 16;
    write_data[5] = payload >> 8;
    write_data[6] = payload;
    write_data[7] = GENERIC_CSS_DEVICE_TRAILER_0;
    write_data[8] = GENERIC_CSS_DEVICE_TRAILER_1;
    ...
    {
        /* Write data */
        bytes = uart_write_port(handle, write_data, GENERIC_CSS_DEVICE_CMD_SIZE);
        #ifdef GENERIC_CSS_CFG_DEBUG
            OS_printf("  GENERIC_CSS_CommandDevice[%d] = ", bytes);
            for (uint32_t i = 0; i < GENERIC_CSS_DEVICE_CMD_SIZE; i++)
            {
                OS_printf("%02x", write_data[i]);
            }
            OS_printf("\n");
        #endif
        if (bytes == GENERIC_CSS_DEVICE_CMD_SIZE)
        {
            status = GENERIC_CSS_ReadData(handle, read_data, GENERIC_CSS_DEVICE_CMD_SIZE);
            if (status == OS_SUCCESS)
            {
                /* Confirm echoed response */
                bytes = 0;
                while ((bytes < (int32_t) GENERIC_CSS_DEVICE_CMD_SIZE) && (status == OS_SUCCESS))
                {
            ...
            {
                #ifdef GENERIC_CSS_CFG_DEBUG
                    OS_printf("GENERIC_CSS_CommandDevice - GENERIC_CSS_ReadData returned %d \n", status);
                #endif
            }
        } 
        else
        {
            #ifdef GENERIC_CSS_CFG_DEBUG
                OS_printf("GENERIC_CSS_CommandDevice - uart_write_port returned %d, expected %d \n", bytes, GENERIC_CSS_DEVICE_CMD_SIZE);
            #endif
        } /* uart_write */
    } /* uart_flush*/
    return status;
