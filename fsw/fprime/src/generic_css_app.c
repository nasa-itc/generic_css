...
void CSS_AppMain(void) {
  ...
  */
  while (CFE_ES_RunLoop(&GENERIC_CSS_AppData.RunStatus) == true) {
    ...
    */
    status =
        CFE_SB_ReceiveBuffer((CFE_SB_Buffer_t **)&GENERIC_CSS_AppData.MsgPtr,
                             GENERIC_CSS_AppData.CmdPipe, CFE_SB_PEND_FOREVER);
    ...
    */
    if (status == CFE_SUCCESS) {
      ...
      CFE_EVS_SendEvent(GENERIC_CSS_PIPE_ERR_EID, CFE_EVS_EventType_ERROR,
                        "GENERIC_CSS: SB Pipe Read Error = %d", (int)status);
      GENERIC_CSS_AppData.RunStatus = CFE_ES_RunStatus_APP_ERROR;
    }
  }
...
int32 GENERIC_CSS_AppInit(void) {
  ...
  */
  ...
  if (status != CFE_SUCCESS) {
    CFE_ES_WriteToSysLog(
        "GENERIC_CSS: Error registering for event services: 0x%08X\n",
        (unsigned int)status);
    return status;
  }
  ...
  */
  status = CFE_SB_CreatePipe(&GENERIC_CSS_AppData.CmdPipe,
                             GENERIC_CSS_PIPE_DEPTH, "CSS_CMD_PIPE");
  if (status != CFE_SUCCESS) {
    CFE_EVS_SendEvent(GENERIC_CSS_PIPE_ERR_EID, CFE_EVS_EventType_ERROR,
  ...
  */
  status = CFE_SB_Subscribe(CFE_SB_ValueToMsgId(GENERIC_CSS_CMD_MID),
                            GENERIC_CSS_AppData.CmdPipe);
  if (status != CFE_SUCCESS) {
  ...
  */
  status = CFE_SB_Subscribe(CFE_SB_ValueToMsgId(GENERIC_CSS_REQ_HK_MID),
                            GENERIC_CSS_AppData.CmdPipe);
  if (status != CFE_SUCCESS) {
  ...
  ** telemetry that has been defined in the GENERIC_CSS_HkTelemetryPkt for this
  *app.
  */
  ...
  *block
  */
  GENERIC_CSS_AppData.HkTelemetryPkt.DeviceEnabled =
      GENERIC_CSS_DEVICE_DISABLED;
  ...
  status = CFE_EVS_SendEvent(
      GENERIC_CSS_STARTUP_INF_EID, CFE_EVS_EventType_INFORMATION,
      "GENERIC_CSS App Initialized. Version %d.%d.%d.%d",
      GENERIC_CSS_MAJOR_VERSION, GENERIC_CSS_MINOR_VERSION,
      GENERIC_CSS_REVISION, GENERIC_CSS_MISSION_REV);
  if (status != CFE_SUCCESS) {
    CFE_ES_WriteToSysLog(
        "GENERIC_CSS: Error sending initialization event: 0x%08X\n",
        (unsigned int)status);
  }
  return status;
}
...
void GENERIC_CSS_ProcessCommandPacket(void) {
  ...
  /*
  ** Ground Commands with command codes fall under the GENERIC_CSS_CMD_MID
  *(Message ID)
  */
  ...
  */
  default:
    ...
    CFE_EVS_SendEvent(GENERIC_CSS_PROCESS_CMD_ERR_EID, CFE_EVS_EventType_ERROR,
                      "GENERIC_CSS: Invalid command packet, MID = 0x%x",
...
void GENERIC_CSS_ProcessGroundCommand(void) {
  ...
  */
  case GENERIC_CSS_NOOP_CC:
    ...
    */
    if (GENERIC_CSS_VerifyCmdLength(GENERIC_CSS_AppData.MsgPtr,
                                    sizeof(GENERIC_CSS_NoArgs_cmd_t)) ==
        OS_SUCCESS) {
      /* Second, send EVS event on successful receipt ground commands*/
      CFE_EVS_SendEvent(GENERIC_CSS_CMD_NOOP_INF_EID,
  ...
  */
  case GENERIC_CSS_RESET_COUNTERS_CC:
    if (GENERIC_CSS_VerifyCmdLength(GENERIC_CSS_AppData.MsgPtr,
                                    sizeof(GENERIC_CSS_NoArgs_cmd_t)) ==
        OS_SUCCESS) {
      CFE_EVS_SendEvent(GENERIC_CSS_CMD_RESET_INF_EID,
                        CFE_EVS_EventType_INFORMATION,
  ...
  */
  case GENERIC_CSS_ENABLE_CC:
    if (GENERIC_CSS_VerifyCmdLength(GENERIC_CSS_AppData.MsgPtr,
                                    sizeof(GENERIC_CSS_NoArgs_cmd_t)) ==
        OS_SUCCESS) {
      CFE_EVS_SendEvent(GENERIC_CSS_CMD_ENABLE_INF_EID,
                        CFE_EVS_EventType_INFORMATION,
  ...
  */
  case GENERIC_CSS_DISABLE_CC:
    if (GENERIC_CSS_VerifyCmdLength(GENERIC_CSS_AppData.MsgPtr,
                                    sizeof(GENERIC_CSS_NoArgs_cmd_t)) ==
        OS_SUCCESS) {
      CFE_EVS_SendEvent(GENERIC_CSS_CMD_DISABLE_INF_EID,
                        CFE_EVS_EventType_INFORMATION,
  ...
  */
  default:
    ...
    CFE_EVS_SendEvent(GENERIC_CSS_CMD_ERR_EID, CFE_EVS_EventType_ERROR,
                      "GENERIC_CSS: Invalid command code for packet, MID = "
                      "0x%x, cmdCode = 0x%x",
...
void GENERIC_CSS_ReportHousekeeping(void) {
  ...
  CFE_SB_TransmitMsg((CFE_MSG_Message_t *)&GENERIC_CSS_AppData.HkTelemetryPkt,
                     true);
  return;
}
...
void GENERIC_CSS_ReportDeviceTelemetry(void) {
  ...
  if (GENERIC_CSS_AppData.HkTelemetryPkt.DeviceEnabled ==
      GENERIC_CSS_DEVICE_ENABLED) {
    status =
        GENERIC_CSS_RequestData(&GENERIC_CSS_AppData.Generic_cssI2c,
                                &GENERIC_CSS_AppData.DevicePkt.Generic_css);
    if (status == OS_SUCCESS) {
      ...
      CFE_SB_TransmitMsg((CFE_MSG_Message_t *)&GENERIC_CSS_AppData.DevicePkt,
                         true);
    } else {
      ...
      CFE_EVS_SendEvent(GENERIC_CSS_REQ_DATA_ERR_EID, CFE_EVS_EventType_ERROR,
                        "GENERIC_CSS: Request device data reported error %d",
                        status);
    }
  }
...
void GENERIC_CSS_Disable(void) {
  ...
  if (GENERIC_CSS_AppData.HkTelemetryPkt.DeviceEnabled ==
    ...
    GENERIC_CSS_AppData.HkTelemetryPkt.DeviceEnabled =
        GENERIC_CSS_DEVICE_DISABLED;
    CFE_EVS_SendEvent(GENERIC_CSS_DISABLE_INF_EID,
                      CFE_EVS_EventType_INFORMATION,
...
*/
int32 GENERIC_CSS_VerifyCmdLength(CFE_MSG_Message_t *msg,
  ...
  if (expected_length == actual_length) {
    ...
    CFE_EVS_SendEvent(
        GENERIC_CSS_LEN_ERR_EID, CFE_EVS_EventType_ERROR,
